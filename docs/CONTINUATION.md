...
## Validação do HUD no Emulicious — 2026-09-05

Foi realizado teste gráfico no Emulicious fornecido, usando a ROM PT-BR gerada pelo `tradutor_mestre_v5.py`. A ROM original americana exibiu corretamente a tela título `SPELLCASTER`, enquanto a ROM PT-BR completa exibiu uma região vermelha/preta no canto superior esquerdo. Foi criada uma variante diagnóstica que preserva as traduções de texto e o repontuamento, mas remove somente as cinco escritas gráficas em `0x9400`, `0x9480`, `0x9680`, `0x97E0` e `0x9A60`; essa variante voltou a exibir o título e `PRESSIONE START` corretamente. A causa das escritas gráficas atuais foi, portanto, validada.

A captura fornecida pelo usuário mostra que o objetivo do HUD é substituir `STGH` por `VIDA` e `ENGY` por `ENRG`. A tentativa automatizada de enviar a tecla `S` ao Emulicious não avançou do título, mesmo enviando o evento às janelas `platform-Emulicious` e `Emulicious - 100%`; isso é limitação do mapeamento da sessão automatizada, não evidência sobre a lógica do jogo. Não se deve distribuir ainda o bloco de cinco escritas gráficas: elas sobrescrevem dados da tela título. A próxima etapa é localizar o stream/tilemap real do HUD ou patchar a tabela de glifos correta em runtime.

A rotina fixa `RST 08h` foi confirmada em `0x0008–0x000E` como configuração do endereço VDP a partir de `DE`. A chamada `0x02E3` lê bytes do stream apontado por `HL` e escreve quatro planos de cada byte no porto VDP, enquanto `0x12AA` carrega um stream de `0xB948` para o destino VDP associado a `0x61A0`. O scanner não encontrou as strings ASCII `STGH`/`ENGY`; elas são índices/glifos runtime, não texto ASCII direto. Não foi feita alteração na ROM do repositório nesta etapa.
